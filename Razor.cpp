...
void Razor::compensate_sensor_errors() {
  ...
  const static double gyroSense = (double) this->getGyroSens();
  const static double magSense = (double) this->getMagSens();

  // compensate accelerometer errors using calibration values
  this->output_packet.acc[0] = ((double) this->ax
      - this->acc_settings.ax_offset) / this->acc_settings.ax_scale;
  this->output_packet.acc[1] = ((double) this->ay
      - this->acc_settings.ay_offset) / this->acc_settings.ay_scale;
  this->output_packet.acc[2] = ((double) this->az
      - this->acc_settings.az_offset) / this->acc_settings.az_scale;

  // get gyroscope values in deg.s^-1
  this->output_packet.gyro[0] = (double)(this->gx)/gyroSense;
  this->output_packet.gyro[1] = (double)(this->gy)/gyroSense;
  this->output_packet.gyro[2] = (double)(this->gz)/gyroSense;

  // Compensate mag error
  // Correct axes orientation first
  this->output_packet.mag[0] = (double)(this->my)/magSense;
  this->output_packet.mag[1] = (double)(this->mx)/magSense;
  this->output_packet.mag[2] = -1.0*((double)(this->mz)/magSense);

  // Mag_readings = M_iron_effects(Rot_w_to_body*Mag_real) - Mag_offset;

  // remove offset from readed data
  this->output_packet.mag[0] = (this->output_packet.mag[0]
      - this->mag_settings.mx_offset);
  this->output_packet.mag[1] = (this->output_packet.mag[1]
      - this->mag_settings.my_offset);
  this->output_packet.mag[2] = (this->output_packet.mag[2]
      - this->mag_settings.mz_offset);

  // copy mag values to temp_mag
  memcpy(this->temp_output_mag, this->output_packet.mag, 3*sizeof(double));

  // multiply by soft iron correction matrix
  matrix_Vector_Multiply(this->mag_settings.mTransform,
      (float*) this->temp_output_mag, this->output_packet.mag);
}
